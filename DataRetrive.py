...
list_FFS = ['PortInFlapRam1_PressRam','PortOutFlapRam1_PressRam','StbdInFlapRam1_PressRam','StbdOutFlapRam1_PressRam','PortInFlapFus_Ang', 'StbdInFlapFus_Ang']
list_autopilot = ['PortFlapAPActive','StbdFlapAPActive']
list_aero = ['StbdJibSheetRam1_Press','PortJibSheetRam1_Press','MainSheetRam_Press', 'MainSheetRam_Pos','PortTravRam_Press','StbdTravRam_Press']
...
def isFloating(df):
    if (df['isTowing'] ==1):
        return 0
    elif((df['FCS_StbdCant_Ang'] < 20) & (df['FCS_PortCant_Ang'] < 20) &  (df['MainSheetRam_Press'] < 50) & (df['StbdJibSheetRam1_Press'] < 50) & (df['PortJibSheetRam1_Press'] < 50)):
        return 1
    else:
        return 0
...
def saveDataSet(dataSet):
    dataSet.to_pickle("full_file_sailing.pkl")
    print("# DONE DataSet Saved #")

def retriveDataSet(reload):
    try:
        if(reload != True):
            dataSet = pd.read_pickle("full_file_sailing.pkl")
        else:
            dataSet = None
    except:
        dataSet = None
